let continuar;
while (continuar != `FIN` && continuar != `Fin` && continuar != `fin`) {
    ...
    if (ok != `ok` && ok != `OK` && ok != `Ok` && ok != `oK`) {
        ...
        if (cantAmigos > 8 || cantAmigos < 2) {
            alert(`Lo sentimos, SDM no admite más de ocho(8) personas en su procesamiento. Tampoco menos de dos(2).`);
        } else {
            let total = parseFloat(prompt(`¿Total del pago?`));
            if (total <= 0) {
                alert(`Lo sentimos, pero ${total} no es un número válido. Debe ser positivo. Por favor, vuelva a empezar.`)
            } else {
                ...
                if (total != totalReal){
                    alert(`El total original estipulado (${total}) difiere del monto sumado entre sus compañeros (${totalReal}). Por favor, inicie de nuevo el programa.`);
                } else {
                    ...
                    continuar = prompt(`Si desea dividir otro gasto y sus deudas, pulse ENTER. Sino, escriba "FIN".`);
                    if (continuar == `FIN` || continuar == `Fin` || continuar == `fin`) {
